...
cp -ur $PREESMFEATURESDIR/* $FEATURESDIR

# Copy all the necessary plugins in the working plugins dir
cp -ur $PREESMPLUGINSDIR/* $PLUGINSDIR
...
# Clean generated stuff from the working plugins dir
...
if [ "$BUILDTYPE" == "tests" ]; then
    PDEBUILDTYPE=I
    REPONAME="PREESM Tests repository"
    NBUILDSTOKEEP=3
elif [ "$BUILDTYPE" == "nightly" ]; then
    PDEBUILDTYPE=N
    REPONAME="PREESM Nightly builds"
    NBUILDSTOKEEP=45
else
    # Release build (not active yet)
    PDEBUILDTYPE=R
    REPONAME="PREESM - Stable releases"
...
ARCHIVEPREFIX=preesm_latest

BUILDID="${BUILDTYPE}_build"

PREESMMAINFEATUREID="org.ietr.preesm.feature"
...
unzip -o $BUILDDIR/$ARTIFACTSFOLDER/$PREESMMAINFEATUREID-$BUILDID.zip -d $LOCALREPO

pushd $LOCALREPO > /dev/null

# Load version number computed from the build
featureFile=$(basename $ARCHIVEPREFIX/features/${PREESMMAINFEATUREID}*)
VERSION=$(echo ${featureFile%.*} | cut -d'_' -f2)
CURRENTBUILD="${PREESMMAINFEATUREID}_${VERSION}"
...
CATEGORY=$PREESMMAINFEATUREID.category.$BUILDTYPE
TMPFILE=$(mktemp)
cat > $TMPFILE <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<site>
    <category-def name="$CATEGORY" label="${REPONAME}"/>
    <feature url="$(ls features/${PREESMMAINFEATUREID}*)" id="${PREESMMAINFEATUREID}" version="${VERSION}">
...
for oldDirectory in $(find -mindepth 1 -maxdepth 1 -type d -name "${PREESMMAINFEATUREID}_*" | sort | head -n -${NBUILDSTOKEEP}); do
...
$P2ADMIN -application org.eclipselabs.equinox.p2.composite.repository \
    -location file:$(pwd) \
    -add $(echo ${PREESMMAINFEATUREID}_* | sed -e "s/ /,/g") \
